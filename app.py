...
if st.button('Fetch Movie Details'):
    if selected_movie_name_input:
        ...
        if movie_details:
            ...
            # Use a fallback poster if the poster is missing or broken
    ...
    if recommendations:
        ...
        for i, movie in enumerate(recommendations):
            with locals()[f'col{i+1}']:
                st.text(movie)
                # Use a placeholder for missing posters
                poster_url = posters[i] if posters[i] != "N/A" else "https://via.placeholder.com/500x750?text=No+Poster+Available"
