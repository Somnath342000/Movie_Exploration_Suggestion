...
if st.button('Recommend'):
    def recommend(movie):
        ...
        return ["Movie A", "Movie B", "Movie C"], ["https://via.placeholder.com/500x750?text=Poster+A", "https://via.placeholder.com/500x750?text=Poster+B", "https://via.placeholder.com/500x750?text=Poster+C"]
    ...
    if recommendations:
        ...
        for i, movie in enumerate(recommendations):
            with locals()[f'col{i+1}']:
                ...
                poster_url = posters[i] if is_valid_image_url(posters[i]) else "https://via.placeholder.com/500x750?text=No+Poster+Available"
                st.image(poster_url, caption=f"Poster of {movie}", use_container_width=True)
    else:
